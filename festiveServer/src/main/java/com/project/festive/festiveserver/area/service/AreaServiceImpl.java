...
public class AreaServiceImpl implements AreaService {
    ...
    public void updateAreaCodes() {
        ...
        try {
            // TourAPI에서 지역코드 조회
            String url = String.format("%s?serviceKey=%s&MobileOS=WEB&MobileApp=festive&_type=json",
                    AREA_CODE_URL, tourApiServiceKey);
            
            TourAreaResponse response = restTemplate.getForObject(url, TourAreaResponse.class);
            ...
            for (TourAreaResponse.Item tourArea : tourAreas) {
                ...
                if (area == null) {
                    ...
                    log.info("지역코드 추가: {} - {}", areaCode, areaName);
                } else if (!areaName.equals(area.getAreaName())) {
                    // 이름이 변경된 경우 업데이트
                    area.setAreaName(areaName);
                    areaRepository.save(area);
                    log.info("지역코드 업데이트: {} - {}", areaCode, areaName);
                }
            }
    ...
    private void updateSigunguCodesForArea(String areaCode) {
        try {
            // TourAPI에서 시군구코드 조회
            String url = String.format("%s?serviceKey=%s&MobileOS=WEB&MobileApp=festive&_type=json&areaCode=%s",
            ...
            // 새로운 시군구코드 Set
            Set<String> newSigunguCodes = tourSigungus.stream()
                    .map(TourAreaResponse.Item::getCode)
                    .collect(Collectors.toSet());
            
            // 기존 시군구코드 Set
            Set<String> existingSigunguCodes = existingSigunguMap.keySet();
            
            // 삭제할 시군구코드
            Set<String> toDelete = existingSigunguCodes.stream()
                    .filter(code -> !newSigunguCodes.contains(code))
                    .collect(Collectors.toSet());
            
            // 삭제 실행
            for (String sigunguCode : toDelete) {
                Sigungu.SigunguId id = new Sigungu.SigunguId();
                id.setAreaCode(areaCode);
                id.setSigunguCode(sigunguCode);
                sigunguRepository.deleteById(id);
                log.info("시군구코드 삭제: {} - {}", areaCode, sigunguCode);
            }
            ...
            for (TourAreaResponse.Item tourSigungu : tourSigungus) {
                ...
                if (sigungu == null) {
                    ...
                    log.info("시군구코드 추가: {} - {} - {}", areaCode, sigunguCode, sigunguName);
                } else if (!sigunguName.equals(sigungu.getSigunguName())) {
                    // 이름이 변경된 경우 업데이트
                    sigungu.setSigunguName(sigunguName);
                    sigunguRepository.save(sigungu);
                    log.info("시군구코드 업데이트: {} - {} - {}", areaCode, sigunguCode, sigunguName);
                }
            }
