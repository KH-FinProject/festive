import React, { useEffect, useState } from "react";
import { Search, Star } from "lucide-react";
import "./AdminMemberManagement.css";
import "./AdminCommon.css";
import AdminSidebar from "./AdminSideBar";
import axiosApi from "../api/axiosAPI";

const AdminMemberManagement = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedMembers, setSelectedMembers] = useState([]);
  const [allMembers, setAllMembers] = useState([]);

  // 전체 회원 데이터
  const fetchAllMembers = async () => {
    try {
      const resp = await axiosApi.get("/admin/allMembers");
      const data = resp.data;

      if (resp.status == 200) {
        setAllMembers(data);
      }
    } catch (err) {
      console.error("전체 회원 불러오기 에러 : ", err);
    }
  };

  // 체크박스 선택/해제 상태 반영
  const handleCheckboxChange = (memberNo) => {
    setSelectedMembers((prev) =>
      prev.includes(memberNo)
        ? prev.filter((number) => number !== memberNo)
        : [...prev, memberNo]
    );
  };

  // 체크박스 전체선택
  const handleSelectAll = () => {
    if (selectedMembers.length === allMembers.length) {
      setSelectedMembers([]);
    } else {
      setSelectedMembers(allMembers.map((member) => member.memberNo));
    }
  };

  // filteredMembers : id, 닉네임, 이름으로 검색된 회원들
  const filteredMembers = (allMembers || []).filter((member) => {
    if (!member) return false;
    return (
      member.id?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      member.name?.includes(searchTerm) ||
      member.nickname?.includes(searchTerm)
    );
  });

  // 삭제 버튼 클릭 시 회원 삭제
  const handleClickDelete = async (memberNoList) => {
    const isConfirmed = window.confirm(
      "회원을 영구 삭제 하시겠습니까? (복구 불가능)"
    );

    if (!isConfirmed) return;
    try {
      const resp = await axiosApi.post("/admin/withdrawDelete", memberNoList);

      if (resp.status == 200) {
        alert(resp.data, "명 삭제되었습니다.");

        setSelectedMembers([]);
        fetchAllMembers();
        return;
      }

      alert("삭제 실패했습니다.");
    } catch (err) {
      console.error("회원 삭제하기 에러 : ", err);
    }
  };

  // 중지 버튼 클릭 시 회원 계정 로그인 중지
  const handleClickDisable = async (memberNoList) => {
    const isConfirmed = window.confirm(
      "해당 회원의 로그인 상태를 변경하시겠습니까?"
    );

    if (!isConfirmed) return;

    try {
      const resp = await axiosApi.post("/admin/memberDisable", memberNoList);

      if (resp.status == 200) {
        alert("해당 계정의 로그인 상태를 변경했습니다.");

        setSelectedMembers([]);
        fetchAllMembers();

        return;
      }

      alert("중지 실패");
    } catch (err) {
      console.error("회원 로그인 중지하기 에러 : ", err);
    }
  };

  useEffect(() => {
    fetchAllMembers();
  }, []);

  return (
    <div className="admin-management-container">
      <div className="management-content">
        {/* Sidebar */}
        <AdminSidebar />
        <div className="content">
          {/* Main Content */}
          <main className="admin-main">
            <div className="admin-header">
              <h1 className="admin-title">전체 회원 관리</h1>
            </div>

            {/* Search Section */}
            <div className="admin-search-section">
              <div className="admin-search-container">
                <input
                  type="text"
                  placeholder="검색할 회원 입력"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="admin-search-input"
                />
                <button className="admin-search-button">
                  <Search className="admin-search-icon" />
                </button>
              </div>
            </div>

            {/* Member List Section */}
            <div className="member-list-section">
              <div className="admin-section-header">
                <h2 className="admin-section-title">
                  회원 정보 ({filteredMembers.length}명)
                </h2>
                {/* 체크박스로 선택한 회원 삭제/제재 */}
                <div className="button-group">
                  <button
                    className="member-action-button member-disable-button"
                    disabled={selectedMembers.length === 0}
                    onClick={() => handleClickDisable(selectedMembers)}
                  >
                    선택 회원 제재
                  </button>
                  <button
                    className="member-action-button member-delete-button"
                    disabled={selectedMembers.length === 0}
                    onClick={() => handleClickDelete(selectedMembers)}
                  >
                    선택 회원 삭제
                  </button>
                </div>
              </div>

              <div className="member-list-container">
                <div className="member-list-header">
                  <div className="member-header-row">
                    <div className="header-cell checkbox-cell">
                      <input
                        type="checkbox"
                        checked={
                          selectedMembers.length === allMembers.length &&
                          allMembers.length > 0
                        }
                        onChange={handleSelectAll}
                        className="member-checkbox"
                      />
                    </div>
                    <div className="header-cell">아이디</div>
                    <div className="header-cell">이름</div>
                    <div className="header-cell">닉네임</div>
                    <div className="header-cell">탈퇴</div>
                    <div className="header-cell action-cell">제재</div>
                    <div className="header-cell action-cell">삭제</div>
                  </div>
                </div>

                <div className="member-list-body">
                  {filteredMembers?.length > 0 ? (
                    filteredMembers.map((member) => (
                      <div key={member.memberNo} className="all-member-row">
                        <div className="member-cell checkbox-cell">
                          <input
                            type="checkbox"
                            checked={selectedMembers.includes(member.memberNo)}
                            onChange={() =>
                              handleCheckboxChange(member.memberNo)
                            }
                            className="member-checkbox"
                          />
                        </div>
                        <div className="member-cell">
                          {member.id == null ? "null" : member.id}
                        </div>
                        <div className="member-cell">{member.name}</div>
                        <div className="member-cell">
                          {member.nickname == null ? "null" : member.nickname}
                        </div>
                        <div className="member-cell">{member.memberDelFl}</div>
                        <div className="member-cell action-cell">
                          <button
                            className="member-action-button member-disable-button"
                            onClick={() =>
                              handleClickDisable([member.memberNo])
                            }
                          >
                            {member.sanctionCount >= 3 ? "제재됨" : "정상"}
                          </button>
                        </div>
                        <div className="member-cell action-cell">
                          <button
                            className="member-action-button member-delete-button"
                            onClick={() => handleClickDelete([member.memberNo])}
                          >
                            삭제
                          </button>
                        </div>
                      </div>
                    ))
                  ) : (
                    <h2 style={{ textAlign: "center", marginTop: "100px" }}>
                      회원이 없습니다.
                    </h2>
                  )}
                </div>
              </div>
            </div>
          </main>
        </div>
      </div>
    </div>
  );
};

export default AdminMemberManagement;
