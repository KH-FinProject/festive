...
const MyPageEditInfo = () => {
  ...
  const fetchMyInfo = useCallback(async () => {
    ...
    try {
      const { data } = await axiosApi.get("/mypage/info", {
  ...
  const getEmailDomain = () =>
    newEmailDomain === "custom" ? customDomain : newEmailDomain;
  ...
  const handleGenerateVerificationCode = async () => {
    ...
    try {
      ...
      alert(
        error.response?.data?.message || "인증번호 발송 중 오류가 발생했습니다."
      );
    } finally {
  ...
  // 이메일 변경 적용 함수
  const handleApplyEmailChange = () => {
    if (!isEmailVerified) return;
    setMemberInfo((prev) => ({
      ...prev,
      email: { local: newEmailLocal, domain: getEmailDomain() },
    }));
  ...
  const handlePhoneMiddleChange = (e) =>
    setNewPhoneMiddle(e.target.value.replace(/[^0-9]/g, ""));
  const handlePhoneLastChange = (e) =>
    setNewPhoneLast(e.target.value.replace(/[^0-9]/g, ""));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axiosApi.post("/mypage/edit-info", updatedData, {
        withCredentials: true,
      });
      alert(data.message);
      fetchMyInfo();
  ...
  const getStatusMessage = (statusObj) => {
    ...
    return (
      <span
        className={`status-message ${statusObj.available ? "success" : "error"
          }`}
      >
        {statusObj.message}
      </span>
  ...
  // 이메일 인증 모달 인증번호 확인 useEffect 추가
  useEffect(() => {
    if (inputCode.length === 6 && !isEmailVerified) {
      const checkEmailAuthKey = async () => {
        try {
          const res = await axiosApi.post("/auth/checkAuthKey", {
            email: `${newEmailLocal}@${getEmailDomain()}`,
            authKey: inputCode,
            authMethod: "email",
          });
          if (res.data.success) {
  ...
  }, [inputCode, newEmailLocal, newEmailDomain, customDomain, isEmailVerified]);

  // 전화번호 인증 모달 인증번호 확인 useEffect 추가
  useEffect(() => {
    if (phoneInputCode.length === 6 && !isPhoneVerified) {
      const checkPhoneAuthKey = async () => {
        try {
          const res = await axiosApi.post("/auth/checkAuthKey", {
            tel: `${newPhoneCarrier}${newPhoneMiddle}${newPhoneLast}`,
            authKey: phoneInputCode,
            authMethod: "tel",
          });
          if (res.data.success) {
      ...
      checkPhoneAuthKey();
    } else if (phoneInputCode.length < 6) {
      setPhoneAuthStatus({ checked: false, available: false, message: "" });
    }
  }, [
    phoneInputCode,
              ...
              >
                ...
                <div className="modal-actions">
                  <button
                    ...
                    onClick={handleApplyEmailChange}
                    disabled={!isEmailVerified}
                  >
                    이메일 변경 적용
                  </button>
              ...
              >
                ...
                <div className="modal-actions">
                  <button
                    type="button"
                    className="modal-button primary"
                    onClick={handleApplyPhoneChange}
                    disabled={!isPhoneVerified}
                  >
                    전화번호 변경 적용
                  </button>
